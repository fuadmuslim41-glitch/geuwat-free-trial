...
const StartJourney: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    // Mock data - always use English learning plan
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-slate-400 font-mono text-xs sm:text-sm">Tekan tombol GEUWAT untuk memulai.</p>
      </header>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="bg-cyan-500/90 hover:bg-cyan-400 disabled:bg-slate-800 disabled:cursor-not-allowed text-slate-950 disabled:text-slate-400 px-8 sm:px-12 py-3 sm:py-4 rounded-lg font-display font-bold flex flex-col items-center justify-center gap-1 transition-all hover:shadow-[0_0_20px_rgba(34,211,238,0.38)] text-sm sm:text-base"
            >
              {loading ? (
                <Loader className="animate-spin w-4 h-4 sm:w-5 sm:h-5" />
              ) : (
                <>
                  <span className="text-lg sm:text-xl">GEUWAT</span>
                  <span className="text-xs sm:text-sm font-normal opacity-80">Bimbing saya</span>
                </>
              )}
            </button>
          </div>
