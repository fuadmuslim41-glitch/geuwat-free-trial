...
function SkillMenuContent() {
  ...
  const { isActive: isGuideActive } = useGuide();
  ...
  const isExecuteReady = !loading && !accessDenied && !accessGranted;
  ...
  const handleSkillClick = (skill: SkillConfig) => {
    ...
    // If guide is active, only allow skill selection change, not navigation
    if (isGuideActive) {
      clearPendingNavigationTimers();
  ...
  return (
    <div className="skill-layout overflow-x-hidden flex flex-col relative font-tech">
      ...
      {/* --- MAIN CONTENT --- */}
      <main className="relative z-10 flex-1 flex flex-col items-center pt-8 md:pt-12 px-4 pb-32 md:pb-36">
             ...
             >
               ...
               style={
                 isExecuteReady
                   ...
                   <div className={`p-3 rounded bg-black/50 border border-gray-700 ${!accessDenied ? `group-hover:${getColorConfig(activeConfig.color).border}` : ''} transition-colors`}>
                      {loading ? (
                   ...
                   <div className="flex flex-col">
                       <span 
                         className={`text-[10px] font-mono tracking-[0.2em] uppercase ${isExecuteReady ? 'skill-status-active-glow' : 'text-gray-500'}`}
                         data-tour="skill-status"
                       >
                           Status: {accessGranted ? 'ACCESS GRANTED' : activeConfig.available ? 'ACTIVE' : 'PREVIEW MODE'}
                       </span>
                       <span className={`text-xl font-display tracking-wide transition-colors duration-300 ${isExecuteReady ? 'skill-execute-text-glow' : ''} ${accessDenied ? 'text-red-500' : accessGranted ? 'text-green-400' : getColorConfig(activeConfig.color).text}`}>
                           {loading ? "INITIALIZING..." : (accessDenied ? "ACCESS DENIED" : accessGranted ? "ACCESS GRANTED" : "EXECUTE")}
                       </span>
                   </div>

                   {/* Corner Decorations */}
                   <div className={`absolute top-0 right-0 w-3 h-3 border-t border-r ${accessDenied ? 'border-red-500' : accessGranted ? 'border-green-400' : getColorConfig(activeConfig.color).border} opacity-50`}></div>
                   <div className={`absolute bottom-0 left-0 w-3 h-3 border-b border-l ${accessDenied ? 'border-red-500' : accessGranted ? 'border-green-400' : getColorConfig(activeConfig.color).border} opacity-50`}></div>
               </div>
             </div>
