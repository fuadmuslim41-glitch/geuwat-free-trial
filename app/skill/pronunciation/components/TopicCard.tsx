...
const TopicCard: React.FC<TopicCardProps> = ({ topic, isActive, onClick }) => {
  ...
  return (
    <div 
      onClick={onClick}
      data-tour={`pronunciation-${topic.id}`}
      className={`
        pronunciation-topic-card
        relative flex-shrink-0 transition-all duration-500 ease-out snap-center
        ${isDisabled 
          ? 'w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 opacity-50 cursor-pointer grayscale mt-6 sm:mt-12 md:mt-16 lg:mt-20' 
          : isActive 
      ...
      `}>
        ...
        <div className={`absolute inset-0 z-20 flex flex-col justify-end p-2 sm:p-3 md:p-4`}>
          ...
          `}>
            {topic.title}
            {isDisabled && (
              <span className="block text-[8px] text-yellow-300/90 mt-1">Trial Preview</span>
            )}
          </h3>
          {isActive && (
