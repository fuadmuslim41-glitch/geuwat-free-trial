...
function resolvePronunBgFromPathname(pathname: string) {
  ...
  if (pathname.startsWith('/skill/pronunciation/locked/')) {
    const topicId = pathname.replace('/skill/pronunciation/locked/', '').split('/')[0] ?? ''
    if (topicId === 'stressing') return PRONUN_PAGE_BG_STRESSING
    if (topicId === 'intonation') return PRONUN_PAGE_BG_INTONATION
    if (topicId === 'final-sound') return PRONUN_PAGE_BG_FINAL_SOUND
    if (topicId === 'american-t') return PRONUN_PAGE_BG_AMERICAN_T
    if (topicId === 'text') return PRONUN_PAGE_BG_TEXT
    if (topicId === 'reading-text') return PRONUN_PAGE_BG_READING_TEXT
